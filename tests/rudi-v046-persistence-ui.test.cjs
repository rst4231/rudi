...
test('CloudStorage backup uses verified double-buffered slots',()=>{
  ...
  assert.match(app,/slot:nextSlot/);
  assert.match(app,/backupTokenChecksum\(restored\)!==checksum/);
  assert.match(app,/readCloudStateBackupToken\(\{allowLegacy:!local\}\)/);
});

test('profile cards do not render redundant role kickers',()=>{
  assert.doesNotMatch(app,/selfLabel\.textContent='Моё'/);
  assert.doesNotMatch(app,/partnerLabel\.textContent='Партнёр'/);
  assert.match(app,/selfCard\.append\(selfIdentity\)/);
  assert.match(app,/partnerCard\.append\(partnerIdentity\)/);
});
  ...
  const cache=createStrictRuntimeCache({
    ...
    getCacheImpl(){
      return {
        ...
        async get(){
          getCalls+=1;
          return getCalls<5?null:{ok:true};
        },
        async delete(){}
      };
    }
