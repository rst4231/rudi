...
test('Telegram products topic is ignored silently and never mutates the shared list', () => {
  ...
  assert.match(block, /products-topic-silent/);
  assert.doesNotMatch(block, /deleteProductsListMessage\(/);
  assert.doesNotMatch(block, /runAuthorizedProductsClear\(/);
});

test('Alice uses RUDI to update the shared products list without Telegram posting', () => {
  const source = fs.readFileSync(entryPath, 'utf8');
