...
test('events runtime retries and isolates Yandex and Stage source failures', () => {
  ...
  assert.match(source, /if\(yandexResult\.status==="fulfilled"\)\{preview\.concerts=concertsMessage/);
  assert.match(source, /messages\.push\(preview\.concerts\)/);
  assert.match(source, /if\(stageResult\.status==="fulfilled"\)\{preview\.stage=stageMessage/);
  assert.match(source, /messages\.push\(preview\.stage\)/);
});
