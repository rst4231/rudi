...
test('Stage text keeps all six events because it is no longer constrained by a photo caption', () => {
  ...
  const formatted = compactEventCaption([
    '<b>🎙 Stage StandUp Club</b>',
    '📅 Суббота, 12 сентября',
  ...
  assert.match(formatted, /6\. Алексей Квашонкин/u);
  assert.doesNotMatch(formatted, /\n…$/u);
  assert.doesNotMatch(formatted, /стоимость уточняйте/u);
  assert.doesNotMatch(formatted, /Основной зал|Красный зал/u);
});
