...
function pickDerivative(photo) {
  ...
  const previews = withSize
    .filter((row) => row.width > 0 && row.height > 0 && Math.max(row.width, row.height) <= 640)
...
module.exports = {
  decodeSetupKey, saveAlbumConfig, readAlbumConfig, initialHost, pickDerivative, fetchLatestPhotos, getLatestPhotos,
};
