...
function installContextualEnvProxy() {
  ...
  const proxy = new Proxy(originalEnv, {
    get(target, property, receiver) { if (property === 'CRON_SECRET' && cronSecretHidden()) return undefined; return Reflect.get(target, property, receiver); },
    has(target, property) { if (property === 'CRON_SECRET' && cronSecretHidden()) return false; return Reflect.has(target, property); },
    ownKeys(target) { const keys = Reflect.ownKeys(target); if (!cronSecretHidden()) return keys; return keys.filter((key) => key !== 'CRON_SECRET'); },
    getOwnPropertyDescriptor(target, property) { if (property === 'CRON_SECRET' && cronSecretHidden()) return undefined; return Reflect.getOwnPropertyDescriptor(target, property); },
  });
  process.env = proxy;
  globalThis[ENV_PROXY_MARKER] = { originalEnv, proxy };
  return globalThis[ENV_PROXY_MARKER];
}
function runWithCronSecretHidden(task) { installContextualEnvProxy(); return cronSecretContext.run({ hideCronSecret: true }, task); }
...
async function telegramRetryDelay(response, fallbackMs = 1000) {
  ...
  try { const payload = await response.clone().json(); const seconds = Number(payload?.parameters?.retry_after); if (Number.isFinite(seconds) && seconds >= 0) return seconds * 1000; } catch {}
  return fallbackMs;
}
function installTelegramFetchGuard(baseFetch = globalThis.fetch, options = {}) {
  ...
  const maxRetryDelayMs = Number.isFinite(options.maxRetryDelayMs) ? Math.max(0, options.maxRetryDelayMs) : 2000;
  let queue = Promise.resolve(); let blockedUntil = 0;
  async function execute(input, init) {
    let attempt = 0;
    while (true) {
      const waitMs = blockedUntil - Date.now(); if (waitMs > 0) await sleep(waitMs);
      const response = await baseFetch(input, init);
      const delayMs = await telegramRetryDelay(response, fallbackRetryMs);
      if (delayMs === null || attempt >= maxRetries || delayMs > maxRetryDelayMs) return response;
      attempt += 1; blockedUntil = Math.max(blockedUntil, Date.now() + delayMs + retryPaddingMs);
    }
  }
