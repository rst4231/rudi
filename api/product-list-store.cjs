...
function categorizeProduct(value) {
  ...
  const rules = [
    ...
    ['Сладкое и снеки', /(шоколад|конфет|печень|вафл|мармелад|зефир|торт|чипс|сухар|орех|батончик|морожен(?:ое|ого|ому|ым|ом|ые|ых|ыми)?\b)/u],
...
function estimateWeeklyAmount(value, category = categorizeProduct(value)) {
  ...
  const specific = [
    ...
    [/(морожен(?:ое|ого|ому|ым|ом|ые|ых|ыми)?\b)/u, '4 шт.'],
