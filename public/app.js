      ...
      function dianaCycleStatus(modelOrPhase){
        ...
        if(phase==='Месячные'){
          ...
          return {
            ...
            advice:'Ориентир по календарю цикла. Реальное настроение и самочувствие Дианы могут быть совсем другими.'
          };
        }
        ...
        if(phase==='Фолликулярная фаза'){
          if(Number.isFinite(day)&&day<=periodLength+2){
            return {
              label:pick(['Бодрая','Лёгкая','Собранная'],day),
              advice:'Ориентировочный статус по дню цикла, а не оценка фактического настроения.'
            };
          }
          if(Number.isFinite(day)&&Number.isFinite(fertileStart)&&day>=fertileStart-2){
            return {
              label:pick(['Энергичная','Активная','Воодушевлённая'],day),
              advice:'По календарю приближается фертильное окно. Статус остаётся только условным ориентиром.'
            };
          }
          return {
            label:pick(['Бодрая','Активная','Собранная','Воодушевлённая'],day),
            advice:'Ориентировочный статус по фазе цикла. Лучше сверяться с тем, как Диана чувствует себя на самом деле.'
          };
        }
        ...
        if(phase==='Фертильное окно'){
          if(Number.isFinite(day)&&Number.isFinite(ovulationDay)&&day===ovulationDay){
            return {
              label:pick(['Яркая','Энергичная','Сияющая'],day),
              advice:'Сегодня ориентировочный день овуляции. Календарь не определяет фактическое настроение или самочувствие.'
            };
          }
          if(Number.isFinite(day)&&Number.isFinite(ovulationDay)&&day<ovulationDay){
            return {
              label:pick(['Энергичная','Яркая','Активная','Общительная'],day),
              advice:'Сейчас расчётное фертильное окно. Статус условный и не является медицинским выводом.'
            };
          }
          return {
            label:pick(['Уверенная','Собранная','Уравновешенная'],day),
            advice:'По календарю овуляция уже прошла. Статус используется только как лёгкий ориентир внутри приложения.'
          };
        }
        ...
        if(phase==='Лютеиновая фаза'){
          if(Number.isFinite(daysToNext)&&daysToNext<=2){
            return {
              label:pick(['Нежная','Спокойная','Чувствительная'],day),
              advice:'До ожидаемых месячных осталось совсем немного. Реальное состояние Дианы важнее календарного статуса.'
            };
          }
          if(Number.isFinite(daysToNext)&&daysToNext<=5){
            return {
              label:pick(['Чувствительная','Вдумчивая','Спокойная'],day),
              advice:'По календарю приближаются месячные. Это только ориентировочный статус.'
            };
          }
          if(Number.isFinite(day)&&Number.isFinite(ovulationDay)&&day<=ovulationDay+3){
            return {
              label:pick(['Уравновешенная','Собранная','Спокойная'],day),
              advice:'Начало лютеиновой фазы. Календарь не определяет фактическое настроение.'
            };
          }
          return {
            label:pick(['Уравновешенная','Вдумчивая','Спокойная','Собранная'],day),
            advice:'Ориентировочный статус по фазе цикла. Фактическое настроение может отличаться.'
          };
        }
